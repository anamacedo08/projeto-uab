...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            login_user(user)
            if user.role == 'admin':
                return redirect(url_for('admin_atendentes'))
            elif user.role == 'atendente':
                return redirect(url_for('atendente_painel'))
            else:
...
@app.route('/clientes/pedidos', methods=['GET', 'POST'])
@login_required
def cliente_pedidos():
...
@app.route('/atendente/iniciar_fabricacao/<int:pedido_id>', methods=['POST'])
@login_required
...
@app.route('/atendente/enviar_pedido/<int:pedido_id>', methods=['POST'])
@login_required
...
@app.route('/admin/atendentes', methods=['GET', 'POST'])
@login_required
...
@app.route('/admin/atendentes/deletar/<int:user_id>', methods=['POST'])
@login_required
